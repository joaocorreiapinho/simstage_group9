...
private:
    ...
    geometry_msgs::Twist calculateCommand()
    {
		...
		float desired_right_wall_distance = 0.18;
        ...
        if(!init_flag){
        	// INIT
        	swtich_case = 0;
        } else if (front_obstacle_distance > 0.25){
        	// PID
        	swtich_case = 1;
        } else{
        	// MANUEVER
        ...
        switch(swtich_case) {
            ...
            case 0 :        // Initialisation
            				if(count < 30){
                                ...
                                count++;
                                std::cout << "SAND STILL\n" ;
                            } else if(front_obstacle_distance > 0.3){
                                msg.linear.x = 0.2;
                                std::cout << "DRIVE FORWARD \n" ;
                            } else {
                                init_flag = true;
                                std::cout << "CHANGED FLAG \n" ;
                            }
                            std::cout << "Case INIT \n" ;
            ...
            case 1 :       	// PI controller
        					...
        					if(front_obstacle_distance < 0.4){
        						msg.linear.x = 0.3;
        					} else{
        						msg.linear.x = 0.5;
        					}
    ...
    {
        right_obstacle_distance = *std::min_element(&msg->ranges [0],&msg->ranges [100]);
        front_obstacle_distance = *std::min_element(&msg->ranges [101], &msg->ranges [140]);
	    left_obstacle_distance = *std::min_element(&msg->ranges [141],&msg->ranges [240]);
